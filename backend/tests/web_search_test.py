...
def make(name, url, *, fail=False, delay=0.0):
    return FakeProvider(
        results=[] if fail else [SearchResult(title=name, url=url, snippet=name, provider=name)],
        fail=fail,
        delay=delay,
        name=name,
    )
...
def test_orchestration() -> None:
    ...
    outcome = asyncio.run(tool.search("q"))
    ...
    check("全部失败也不抛异常", asyncio.run(tool.search("q")).results == [])
