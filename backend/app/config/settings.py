...
class Settings(BaseSettings):
    ...
    # 对话性问题（你是谁 / 你是做什么的 / 介绍一下你自己…）的**更高**相关性门槛：
    # 标定依据 —— 这类寒暄问句几乎不含专业实词，与知识文档通常只共享"是/什/么"
    # 这类常见字；在内置离线哈希向量下，它们的余弦相似度通常低于 0.30
    # （实测无关片段落在 0.18~0.25，恰好越过通用阈值，造成"假命中"）。
    # 因此把"是否把片段纳入自我介绍上下文"的门槛从 0.18 提高到 0.30；
    # 换用外部语义向量服务后该值同样需要重新标定。
    self_intro_min_score: float = 0.30
